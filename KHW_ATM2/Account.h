...
public:
	...
	int deposit(int id, string password, int money); //입금하기
	int widraw(int id, string password, int money);	//출금하기
	int deposit(int id, int money);	//계좌 이체
	int getBalance() { return nBalance; } //잔액 불러오기
	string getAccountName() { return strAccountName; } //고객 명 불러오기
};
#endif